function K = get_k_length(leg_length)
    ...
    R1=0.06;                         %驱动轮半径已更改为实车，sw测（扯皮半天。。。）
    %R1=0.054;                         %驱动轮半径已更改为实车，非常准确

    L1=leg_length/2;                  %摆杆重心到驱动轮轴距离不用管
    LM1=leg_length/2;                 %摆杆重心到其转轴距离不用管

    l1=0.025;                          %机体质心距离转轴距离       存疑
    %测量得到的是0.015，但是发散，疑似这里问题，改成能站立时候的数据
    %l1=0.05;                          %机体质心距离转轴距离不太准确

    mw1=0.161;                         %驱动轮质量sw测单个轮子的
    %mw1=0.20;                         %驱动轮质量，不知道

    mp1=2.034;                         %杆质量单腿，sw测
    %mp1=1.00;                         %杆质量不太准确

    M1=9.25;                           %机体质量整车18.5，分开建模÷2  sw测
    %M1=18.5;                           %机体质量已更改，不太准确

    Iw1=mw1*R1^2;                     %驱动轮转动惯量不用管
    Ip1=mp1*((L1+LM1)^2+0.05^2)/12.0; %摆杆转动惯量

    %IM1=M1*(0.3^2+0.12^2)/12.0;       %机体绕质心转动惯量
    
    IM1=0.20547;       %机体绕质心转动惯量整车0.41094，分开建模÷2  sw测
    %IM1=0.53434;       %机体绕质心转动惯量，不太准确
    ...
    Q=diag([100 1 500 200 10000 100])
